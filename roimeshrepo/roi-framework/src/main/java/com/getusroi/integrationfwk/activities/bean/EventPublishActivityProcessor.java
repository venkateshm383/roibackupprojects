...
public class EventPublishActivityProcessor {
	...
	/**
	 * This method is used to process the event publish pipeline activity, by associating the pipeline activity data with event param
	 * of event configuration
		...
		String  eventname=eventPublishActivity.getEventName();
			try {
				...
				if(event!=null){
					...
					throw new EventPublishActivityException("event configuration defined by event name "+eventname+" is null");
				}
	...
	/**
	 * This method is usd to create pipeline activity event and store in cache,so that it should be dispatched later
	...
	private void createAndStorePipelineEventInCache(Event event,Exchange exchange) {
		...
		logger.debug("addEventParamFromEventPipeActivity requestId=" + requestId + "--hcTransactionContext=" + hcTransactionContext);
	...
	/**
	 * This method is used to search event configuration defined in configuration file from cache using event name
	 ...
	 * @throws EventPublishActivityException 
	 */
	private Event searchEventConfigurationByEventName(String eventname, Exchange exchange) throws EventFrameworkConfigurationException, EventPublishActivityException {
