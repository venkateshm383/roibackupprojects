...
public class PermaStoreConfigurationService extends GenericApplicableNode implements IPermaStoreConfigurationService {
	...
	public void addPermaStoreConfiguration(ConfigurationContext configurationContext, PermaStoreConfiguration psConfig)
		...
		try {
			...
			if((vendorName != null && !(vendorName.isEmpty()) && !(vendorName.equalsIgnoreCase("")))&&(version != null && !(version.isEmpty()) && !(version.equalsIgnoreCase("")))){
				configNodeId = getApplicableNodeIdVendorName(tenantId, siteId,configurationContext.getFeatureGroup(),configurationContext.getFeatureName(),vendorName,version);
			}else{
				configNodeId = getApplicableNodeIdFeatureName(tenantId, siteId,configurationContext.getFeatureGroup(),configurationContext.getFeatureName());
			}
				logger.debug("Applicable Config Node Id is =" + configNodeId);
		
				// Get the type of Configuration Bulder and get the DatatoCache from the
				// Builder
				...
				ConfigNodeData loadedConfigNodeData = configPersistenceService.getConfigNodeDatabyNameAndNodeId(configNodeId,psConfig.getName(),PermaStoreConfigurationConstant.PERMASTORE_CONFIG_TYPE);
				int configDataId=0;
				...
				if (loadedConfigNodeData == null) {
					...
					throw new PermaStoreConfigurationException("PermaStoreConfiguration already exist for ConfigName="
							...
							+ featureInfo.getFeatureGroup() + "/" + featureInfo.getFeatureName());
				}
				...
				PermaStoreConfigurationUnit psConfigUnit = new PermaStoreConfigurationUnit(tenantId, siteId,configNodeId,isConfigEnabled, psConfig, objToCache);
			...
			throw new PermaStoreConfigurationException("Failed to insert ConfigData in DB for configName="
					+ psConfig.getName(), sqlExp);
		}
	}
	...
	public int updatePermaStoreConfiguration(ConfigurationContext configurationContext,PermaStoreConfiguration psConfig, int configNodedataId)throws PermaStoreConfigurationException, PermaStoreConfigParserException{
		...
		try {
			...
			if((vendorName != null && !(vendorName.isEmpty()) && !(vendorName.equalsIgnoreCase("")))&&(version != null && !(version.isEmpty()) && !(version.equalsIgnoreCase("")))){
				...
				ConfigNodeData configNodeData = new ConfigNodeData();
				configNodeData.setConfigName(psConfig.getName());
				//#TODO Enable or Disable should come from config need to add it in XMLSchema
				configNodeData.setEnabled(psConfig.isIsEnabled());
				boolean isConfigEnabled=psConfig.isIsEnabled();
				configNodeData.setConfigLoadStatus("Sucess");
				configNodeData.setConfigType(PermaStoreConfigurationConstant.PERMASTORE_CONFIG_TYPE);
				...
				IConfigPersistenceService configPersistenceService = new ConfigPersistenceServiceMySqlImpl();
				logger.debug("before updating the permastore configNodeData "+configNodeData  +"configId "+configNodedataId);
				...
				try{
					...
					permaStoreRequestContext=new RequestContext(tenantId, siteId, psConfig.getFeatureInfo().getFeatureGroup(),psConfig.getFeatureInfo().getFeatureName());
					changeStatusOfPermaStoreConfig(configurationContext, psConfig.getName(), isConfigEnabled);
					}else{
						permaStoreRequestContext=new RequestContext(tenantId, siteId, psConfig.getFeatureInfo().getFeatureGroup(),null);
						changeStatusOfPermaStoreConfig(configurationContext,  psConfig.getName(), isConfigEnabled);
		...
		try {
			...
			if((vendorName != null && !(vendorName.isEmpty()) && !(vendorName.equalsIgnoreCase("")))&&(version != null && !(version.isEmpty()) && !(version.equalsIgnoreCase("")))){
			 nodeId = getApplicableNodeIdVendorName(requestContext.getTenantId(), requestContext.getSiteId(),
					requestContext.getFeatureGroup(), requestContext.getFeatureName(),requestContext.getVendor(),requestContext.getVersion());
			}else{
			...
			logger.debug(".getPermaStoreConfiguration(reqContext=" + requestContext + ",configName="+configName+")");
			...
			if (permaStoreConfigUnit == null && requestContext.getFeatureName() != null) {
				...
				permaStoreConfigUnit = (PermaStoreConfigurationUnit) configServer.getConfiguration(
						requestContext.getTenantId(), psGroupKey, configName);
			}
	...
	public boolean reloadPerStoreCacheObject(RequestContext requestContext, String configName) throws PermaStoreConfigurationException {
		...
		try {
			...
			ConfigNodeData configNodeData=configPersistenceService.getConfigNodeDatabyNameAndNodeId(configUnit.getAttachedNodeId(), configName, PermaStoreConfigurationConstant.PERMASTORE_CONFIG_TYPE);
			...
			logger.error("Failed to reLoad ConfigurationUnit from cache it either not exist or is disabled with Name="+configName,e);
			throw new PermaStoreConfigurationException("Failed to reLoad ConfigurationUnit from cache it either not exist or is disabled with Name="+configName,e);
	...
	/**
	 * Finds the Respective PermaStoreConfiguration and Deletes it from the DataGrid as well as from the DataBase<BR>
	 * Note:-To purge/remove only from DataGrid use .changeStatusOfPermaStoreConfig() marking status as disabled.
	 * @param requestContext
	 * @param configName
	 * @return
	 * @throws PermaStoreConfigRequestException
	 */
	public boolean deletePermaStoreConfiguration(ConfigurationContext configurationContext, String configName)throws PermaStoreConfigurationException {
		logger.debug(".deletePermaStoreConfiguration(reqContext=" + configurationContext + ",configName="+configName+")");
			RequestContext reqContext=new RequestContext(configurationContext.getTenantId(),configurationContext.getSiteId(),configurationContext.getFeatureGroup(),configurationContext.getFeatureName(),configurationContext.getVendorName(),configurationContext.getVersion());
		try {
			...
			logger.debug(".deletePermaStoreConfiguration() deleted from DataGrid psGroupKey="+psGroupKey+" configName="+configName);
			
			return true;
		} catch (ConfigPersistenceException | PermaStoreConfigRequestException | ConfigServerInitializationException e) {
	 ...
	 * @throws PermaStoreConfigurationException
	 */
	private boolean deletePermaStoreConfigurationFromDb(String configName,
			int nodeId) throws PermaStoreConfigurationException {
		// Delete from the DB
		IConfigPersistenceService configPersistenceService = new ConfigPersistenceServiceMySqlImpl();
				logger.debug(".deletePermaStoreConfigurationFromDb() deleted from db configName="
				+ configName);
		try {
	...
	public void changeStatusOfPermaStoreConfig(ConfigurationContext configContext, String configName,
			boolean isEnable) throws PermaStoreConfigurationException {
		RequestContext requestContext=new RequestContext(configContext.getTenantId(),configContext.getSiteId(),configContext.getFeatureGroup(),configContext.getFeatureName(),configContext.getVendorName(),configContext.getVersion());
		try{
			
			Integer applicableNodeId=getApplicableNodeId(requestContext);
			IConfigPersistenceService configPersistenceService = new ConfigPersistenceServiceMySqlImpl();
			ConfigNodeData configNodeData=configPersistenceService.getConfigNodeDatabyNameAndNodeId(applicableNodeId, configName, PermaStoreConfigurationConstant.PERMASTORE_CONFIG_TYPE);
			if(configNodeData==null){
				//Not in DB so it does not exist throw exception
				throw new PermaStoreConfigurationException("PermaStoreConfig with Name( "+configName+") does not exist in DB");
			}
			...
			if(!isEnable){
				...
				//We have to Disable psConfig hence remove from DataGrid and update DB as disabled Configuration
				configPersistenceService.enableConfigNodeData(false,configNodeData.getNodeDataId());
							
				//Now remove from DataGrid
				String psGroupKey= PermaStoreConfigurationUnit.getConfigGroupKey(configNodeData.getParentConfigNodeId());
				ROIConfigurationServer configServer = ROIConfigurationServer.getConfigurationService();
			...
			throw new PermaStoreConfigurationException("Failed to Enable/Disable PermaStoreConfig with name "+configName,e);
		} 
	...
	private void enableAndLoadPermaStroreConfig(RequestContext reqCtx,ConfigNodeData configNodeData) throws ConfigPersistenceException, PermaStoreConfigParserException, PermaStoreConfigurationException{
		IConfigPersistenceService configPersistenceService = new ConfigPersistenceServiceMySqlImpl();
		...
		PermaStoreConfigurationUnit psConfigUnit = new PermaStoreConfigurationUnit(reqCtx.getTenantId(), reqCtx.getSiteId(),configNodeData.getParentConfigNodeId(), true, psConfig, objToCache);
		psConfigUnit.setDbconfigId(configNodeData.getNodeDataId());
		loadConfigurationInDataGrid(psConfigUnit);
		
	}
	
	private void loadConfigurationInDataGrid(PermaStoreConfigurationUnit psConfigUnit ) throws PermaStoreConfigurationException{
		//PermaStoreConfigurationUnit psConfigUnit = new PermaStoreConfigurationUnit(tenantId, siteId,configNodeId, true, psConfig, objToCache);
		//psConfigUnit.setDbconfigId(configDataId);
		
		logger.debug(".loadConfigurationInDataGrid() PermaStoreConfigurationUnit=" + psConfigUnit);
		try {
			ROIConfigurationServer configServer = ROIConfigurationServer.getConfigurationService();
			//I don't want to store the PermaStoreConfiguration in the DataGrid Cache hence setting it as null
			...
			throw new PermaStoreConfigurationException("Failed to Upload in DataGrid configName="
					+ psConfigUnit.getKey(), e);
		}
	}
	
	

	private void isPermastoreRequestValid(RequestContext requestContext)
			throws PermaStoreConfigRequestException {
	...
	/**
	 *
	 * based on the PermaStoreRequestContext and config name searching permastoreconfig ,if permastoreconfig Exist in DB check wether it enabled or not ,
	...
	public boolean checkPermaStoreConfigarationExistOrNot(ConfigurationContext configurationContext,String configName)
		...
		logger.debug("Inside checkPermaStoreConfigarationExistOrNot method with configurationContext = "+configurationContext  +" configName = "+configName);
		RequestContext requestContext=new RequestContext(configurationContext.getTenantId(),configurationContext.getSiteId(),configurationContext.getFeatureGroup(),configurationContext.getFeatureName(),configurationContext.getVendorName(),configurationContext.getVersion());
		...
		IConfigPersistenceService configPersistenceService = new ConfigPersistenceServiceMySqlImpl();
		ConfigNodeData configNodeData=configPersistenceService.getConfigNodeDatabyNameAndNodeId(featureNodeId, configName, PermaStoreConfigurationConstant.PERMASTORE_CONFIG_TYPE);
		...
		try{
			...
			throw new PermaStoreConfigurationException("Error in Loading the PermastoreConfig to cache with configName = "+configName +" Request Context = "+requestContext,e);
		}
			
		}
		} catch (ConfigPersistenceException | InvalidNodeTreeException e) {
			throw new PermaStoreConfigRequestException(
					"Error in searching PermastoreCongaration with configName = "+configName +" Request Context = "+requestContext);		
	}
		return true;
	}
