...
*/
	...
	public Integer getApplicableNodeId(String tenantId, String siteId, String featureGroup, String featureName,String vendorName,String version) throws InvalidNodeTreeException,
			...
			if (featureName == null) {
				...
				if(fconfigNodeTree != null && fconfigNodeTree.getNodeName().equalsIgnoreCase(featureName)){
					if(vendorName == null ){
						...
						List<ConfigurationTreeNode> featureTreeNodeList = fconfigNodeTree.getChildNodes();
						logger.debug("featureTreeNodeList : "+featureTreeNodeList);
						Integer nodeId=getVendorTreeNodeId(featureTreeNodeList, vendorName, version);
						//we are geting the node if for specific tenant,site,featuregroup,feature,vendor,version(optional)
		...
		throw new InvalidNodeTreeException("Not able to find NodeTree in DB for Tree=" + tenantId + "/" + siteId + "/" + featureGroup + "/" + featureName+"/"+vendorName+"/"+version);
	}
