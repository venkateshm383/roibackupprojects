...
public class FeatureConfigurationService extends GenericApplicableNode implements IFeatureConfigurationService{
	...
	public void addFeatureConfiguration(ConfigurationContext configContext, Feature feature) throws FeatureConfigurationException {
		...
		try {
			IFeatureMasterService featureMasterService=new FeatureMasterService();
			Integer configNodeId=0;
			if(vendorName != null && !(vendorName.isEmpty())){
				configNodeId = getApplicableNodeId(tenantId,siteId,groupName,vendorName,version,feature);
			}else{
			 configNodeId = getApplicableNodeId(tenantId, siteId, groupName, feature);
			}
			...
			if (loadedConfigNodeData == null) {
				...
				throw new FeatureConfigurationException("FeatureConfiguration already exist for ConfigName=" + feature.getFeatureName() + "--tree="
						+ tenantId + "/" + siteId + "/" + groupName + "/" + feature.getFeatureName());
			}
	...
	public FeatureConfigurationUnit getFeatureConfiguration(FeatureConfigRequestContext requestContext, String configName)
		...
		try {
			...
			//logger.error("time taken to get ROIConfigurationServer  :: " + System.currentTimeMillis());
			...
			throw new FeatureConfigurationException("Unable to get the feature for tenant : " + requestContext.getTenantId() + ", site : "
					...
					+ requestContext.getFeatureName() + ", and configname : " + configName, e);
		}
	}
	...
	public void addNewServiceInFeatureConfiguration(ConfigurationContext configContext, Service service)
		...
		FeatureConfigRequestContext featureRequestContext=new FeatureConfigRequestContext(configContext.getTenantId(), configContext.getSiteId(), configContext.getFeatureGroup(),configContext.getFeatureName(),configContext.getVendorName(),configContext.getVersion());
		try {
	...
	public int updateFeatureConfiguration(ConfigurationContext configContext, String groupName, Feature fsConfig, int configNodedataId)
		...
		try {
			...
			if (fsConfig.getFeatureName() != null) {
				featureRequestContext = new FeatureConfigRequestContext(tenantId, siteId, groupName, fsConfig.getFeatureName());
	...
	public void changeStatusOfFeatureConfig(ConfigurationContext configContext, String featureName, boolean isConfigEnabled)
		...
		try {
			...
			if (!isConfigEnabled) {
				...
				enableAndLoadFeatureConfig(featureRequestContext, configNodeData);
			}
	...
	public boolean deleteFeatureConfiguration(ConfigurationContext configContext, String configName) throws FeatureConfigurationException {
		...
		FeatureConfigRequestContext reqContext=new FeatureConfigRequestContext(configContext.getTenantId(), configContext.getSiteId(), configContext.getFeatureGroup(),configContext.getFeatureName(),configContext.getVendorName(),configContext.getVersion());
		try {
			...
			if (fsconfigUnit == null) {
				...
				// delete from DB
				Integer configNodeId = getApplicableNodeIdFeatureName(reqContext.getTenantId(), reqContext.getSiteId(), reqContext.getFeatureGroup(),
			...
			FeatureConfigurationUnit fsconfigUnit1 = getFeatureConfiguration(reqContext, configName);
			logger.debug("featureCOnfiguration Unit after delete : " + fsconfigUnit1);
			return true;
	...
	public boolean checkFeatureExistInDBAndCache(ConfigurationContext configContext, String configName)throws  FeatureConfigurationException,FeatureConfigRequestException {
		FeatureConfigRequestContext requestContext=new FeatureConfigRequestContext(configContext.getTenantId(), configContext.getSiteId(), configContext.getFeatureGroup(),configContext.getFeatureName(),configContext.getVendorName(),configContext.getVersion());
		...
		try {
			...
			if(vendorName != null && !(vendorName.isEmpty())){
				...
				featureNodeId=getApplicableNodeIdVendorName(requestContext.getTenantId(), requestContext.getSiteId(), requestContext.getFeatureGroup(), requestContext.getFeatureName(),requestContext.getVendor(),requestContext.getVersion());

			}else{
				logger.debug("getting the node id till feature level");
			featureNodeId=getApplicableNodeIdFeatureName(requestContext.getTenantId(), requestContext.getSiteId(), requestContext.getFeatureGroup(), requestContext.getFeatureName());
			}
