...
import com.getusroi.feature.config.impl.FeatureDiscoveryService;
...
public class FeatureMetaInfoResourceUtil {
	...
	public void checkResourceAvailableAndload(Feature feature,Bundle bundle,String featureGroupName) throws FeatureMetaInfoResourceException {
		...
		com.getusroi.featuremetainfo.jaxb.PermaStoreConfigurations permastoreConfiguration=feature.getPermaStoreConfigurations();
	 ...
	 * @throws FeatureMetaInfoResourceException
	 */
	private void checkPermastoreResourceAndLoad(com.getusroi.featuremetainfo.jaxb.PermaStoreConfigurations permastoreConfiguration,Bundle bundle,Feature feature, String featureGroupName) throws FeatureMetaInfoResourceException{
		...
		for (EventResource eventresource : eventResourceList) {
			...
			if (eventresource != null) {
				...
				try {
					...
					ConfigurationContext configContext = new ConfigurationContext(MeshHeaderConstant.tenant, MeshHeaderConstant.site,
				...
				for (EventDispatcher eventDispacher : eventDispacherList) {
					...
					if (transformationtype.equalsIgnoreCase("CUSTOM")) {
						...
						try {
							...
							IROIEventTransformer roiEventTransformer = (IROIEventTransformer) classObj.newInstance();
							Dictionary<String, String> props = new Hashtable<String, String>();
							props.put("tenant", MeshHeaderConstant.tenant);
							props.put("site", MeshHeaderConstant.site);
							...
							bundle.getBundleContext().registerService(IROIEventTransformer.class.getName(), roiEventTransformer, props);
			...
			if (permastoreSourceAsString != null) {
				...
				try {
					RequestContext requestContext = new RequestContext(MeshHeaderConstant.tenant,
							MeshHeaderConstant.site, featureGroupName, featureName,vendor,version);
					permastorConfig = parmastoreConfigParser.marshallConfigXMLtoObject(permastoreSourceAsString);
					List<com.getusroi.permastore.config.jaxb.PermaStoreConfiguration> permastoreConfigList1 = permastorConfig.getPermaStoreConfiguration();
	 ...
	 * @throws FeatureMetaInfoResourceException
	 */
	private void loadingPermastoreCustomBuildType(com.getusroi.permastore.config.jaxb.PermaStoreConfiguration permaStoreConfiguration,RequestContext requestContext,String configname,Bundle bundle) throws FeatureMetaInfoResourceException{
			...
			if (policyconfigSourceAsString != null) {
				...
				try {
					...
					for (Policy policy : policyList) {
						...
						PolicyRequestContext policyRequestContext = new PolicyRequestContext(MeshHeaderConstant.tenant,
								MeshHeaderConstant.site, featureGroupName, featureName,vendor,version);
			...
			if (featureImplSourceAsString != null) {
				...
				try {
					...
					com.getusroi.feature.jaxb.Feature feature1 = feaureServiceInfo.getFeatures().getFeature();				
						...
						FeatureConfigRequestContext requestContext = new FeatureConfigRequestContext(MeshHeaderConstant.tenant,
								MeshHeaderConstant.site, featureGroupName, featureName,vendor,version);
						ConfigurationContext configContext=new ConfigurationContext(requestContext);
	 ...
	 * @throws FeatureMetaInfoResourceException
	 */
	private void checkPermastoreResourceAndDelete(com.getusroi.featuremetainfo.jaxb.PermaStoreConfigurations permastoreConfiguration,Bundle bundle,Feature feature, String featureGroupName) throws FeatureMetaInfoResourceException{
		if(permastoreConfiguration !=null){
			List<PermaStoreConfiguration> permastoreConfigList = permastoreConfiguration.getPermaStoreConfiguration();
			if (permastoreConfigList != null) {
			...
			if(dynaResourceUrl !=null){
				...
				try {
					DynastoreConfigurations dynastoreConfigurations =parser.marshallConfigXMLtoObject(dynastoreAsSourceAsString);
					 ...
					 RequestContext dynaStoreConfigRequestContext = new RequestContext(MeshHeaderConstant.tenant, MeshHeaderConstant.site,
								featureGroupName, name,vendor,version);
					iDynaStoreConfigurationService= new DynaStoreConfigurationService();
					List<DynastoreConfiguration> dynastoreConfigList=dynastoreConfigurations.getDynastoreConfiguration();
					for(DynastoreConfiguration dynaConfig:dynastoreConfigList){
			...
			if (permastoreSourceAsString != null) {
				...
				try {
					...
					for (com.getusroi.permastore.config.jaxb.PermaStoreConfiguration permaStoreConfiguration : permastoreConfigList1) {
						String configname = permaStoreConfiguration.getName();
						IPermaStoreConfigurationService psConfigService = new PermaStoreConfigurationService();
						RequestContext requestContext = new RequestContext(MeshHeaderConstant.tenant, MeshHeaderConstant.site,
								featureGroupName, name,vendor,version);
						ConfigurationContext configContext=new ConfigurationContext(requestContext);
						try {
							boolean isExist = psConfigService.checkPermaStoreConfigarationExistOrNot(configContext, permaStoreConfiguration.getName());
			...
			if (featureImplSourceAsString != null) {
				...
				try {
					...
					com.getusroi.feature.jaxb.Feature feature1 = feaureServiceInfo.getFeatures().getFeature();				
						...
						IFeatureConfigurationService featureConfigService = new FeatureConfigurationService();
						FeatureConfigRequestContext requestContext = new FeatureConfigRequestContext(MeshHeaderConstant.tenant,
								MeshHeaderConstant.site, featureGroupName, feature1.getFeatureName(),vendor,version);
						ConfigurationContext configContext=new ConfigurationContext(requestContext);
						try {
							boolean isExist = featureConfigService.checkFeatureExistInDBAndCache(configContext, feature1.getFeatureName());
							if (isExist) {
