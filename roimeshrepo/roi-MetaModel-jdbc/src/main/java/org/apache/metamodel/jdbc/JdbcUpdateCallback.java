...
abstract class JdbcUpdateCallback extends AbstractUpdateCallback implements UpdateCallback {
    ...
    public final void close(boolean success) {
        if (_connection != null) {
      	  //System.out.println("if in global transaction dont close prepared statement status="+_dataContext.isInTransaction()); 
      	  //if (success && _preparedStatement != null && !_dataContext.isInTransaction()) {
      	  if (success && _preparedStatement != null) {
                closePreparedStatement(_preparedStatement);
            }
            //System.out.println("if in global transaction dont set auto commit status="+_dataContext.isInTransaction()); 
            //if (_dataContext.getQueryRewriter().isTransactional()) {
              
            	try {
                    commitOrRollback(success);

                    if (_dataContext.isDefaultAutoCommit()&& !_dataContext.isInTransaction()) {
                        try {
    ...
    private void commitOrRollback(boolean success) {
        if (success) {
            try {
            	 if(_dataContext.isInTransaction()){
            		 //you are in Global transaction do nothing
            		 System.out.println("JDBCUpdateCallback.commitOrRollback() in Global transaction not commiting");
            	 }else{
            ...
            try {
            	 if(_dataContext.isInTransaction()){ 
            		 //you are in Global transaction do nothing
            		 System.out.println("JDBCUpdateCallback.commitOrRollback() in Global transaction throw error dont rollback");
            	 }else{
